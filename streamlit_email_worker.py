...
#                 query = """
#                     SELECT
#                         a.id, a.data, a.hora, a.created_at,
...
#             self.logger.info(
#                 f"Verificação concluída: {sum(resultados.values()[:3])} emails enviados")
...
#         while self.running:
#             try:
#                 # Verificar apenas se houve atividade recente no app
#                 if hasattr(st.session_state, 'last_activity'):
#                     ultima_atividade = st.session_state.last_activity
#                     if datetime.now() - ultima_atividade > timedelta(minutes=30):
#                         # App inativo há mais de 30 min, pausar worker
#                         time.sleep(60)
#                         continue
...
#                     self.processar_envios(0)  # Confirmações
#                     self.processar_envios(1)  # Lembretes 1 dia
#                     self.processar_envios(2)  # Lembretes 30 min
...
#     with col1:
#         status = "🟢 Ativo" if worker.running else "🔴 Parado"
#         st.metric("Status do Worker", status)
...
#     with col3:
#         st.metric("Total de Emails", stats["total"])
...
#         • A cada 5 minutos (quando o minuto é divisível por 5)
#         • Cada tipo de email é enviado apenas uma vez por agendamento
#         • Sistema evita emails duplicados através da tabela `emails_enviados`

#         **Nota**: O sistema não depende mais da coluna `envio` dos agendamentos!
#         """)
...
#         if logs_content:
#             # Mostrar apenas últimas 30 linhas
...
#     st.title("Sistema de Barbearia - Email Worker v2.0")

#     # Inicializar sistema de emails automaticamente
#     exibir_interface_email_worker()

#     st.markdown("---")
#     st.success("✨ O sistema de emails está rodando baseado em tempo real!")
#     st.info("Agora os emails são enviados independente do status da coluna 'envio'!")
import streamlit as st
import sqlite3
import smtplib
import threading
...
class StreamlitEmailWorker:
    ...
    def worker_background(self):
        ...
        while self.running:
            try:
                # CORREÇÃO 2: Remover/comentar verificação de atividade restritiva que pausava o worker
                # PROBLEMA: Este bloco pausava o worker quando não havia atividade
                """
                if hasattr(st.session_state, 'last_activity'):
                    ultima_atividade = st.session_state.last_activity
                    if datetime.now() - ultima_atividade > timedelta(minutes=30):
                        # App inativo há mais de 30 min, pausar worker
                        time.sleep(60)
                        continue  # <-- ESTE continue IMPEDIA A EXECUÇÃO!
                """

                # CORREÇÃO 3: Executar mais frequentemente
                agora = datetime.now()

                # ANTES: Executar apenas a cada 5 minutos
                # if agora.minute % 5 == 0:

                # DEPOIS: Executar a cada 2 minutos E evitar execuções muito próximas
                deve_executar = False
